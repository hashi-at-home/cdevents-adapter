...
describe('GitHub Adapter Essential Tests', () => {
  ...
  describe('Webhook Data Validation - Required Fields Only', () => {
    ...
    const createCompleteWebhook = (
      ...
      workflow_job: {
        ...
        head_branch: 'main',
        run_url:
          'https://api.github.com/repos/owner/repo/actions/runs/987654321',
        run_attempt: 1,
  ...
  describe('CDEvent Transformation - workflow_job.in_progress', () => {
    it('should transform in_progress webhook to valid PipelineRunStarted CDEvent', async () => {
      ...
      const cdevent = await adapter.transform(
        webhook,
        'workflow_job.in_progress'
      );
  ...
  describe('CDEvent Transformation - workflow_job.waiting', () => {
    it('should transform waiting webhook to valid PipelineRunQueued CDEvent', async () => {
      ...
      expect(cdevent.context.type).toBe(
    ...
    it('should generate consistent subject IDs for the same workflow job', async () => {
      const baseWebhook = {
        ...
        workflow_job: {
          ...
          steps: [],
          check_run_url:
            'https://api.github.com/repos/owner/repo/check-runs/999',
          labels: [],
          runner_id: null,
          runner_name: null,
          runner_group_id: null,
      ...
      const cdevent1 = await adapter.transform(
        baseWebhook,
        'workflow_job.queued'
      );
      const cdevent2 = await adapter.transform(
        webhook2,
        'workflow_job.in_progress'
      );
    ...
    it('should construct correct source URI from repository information', async () => {
      const webhook = {
        ...
        workflow_job: {
          ...
          head_branch: 'main',
          run_url:
            'https://api.github.com/repos/my-org/my-repo/actions/runs/456',
          run_attempt: 1,
          node_id: 'MDExOldvcmtmbG93Sm9iMTIz',
          head_sha: 'abc123',
          url: 'https://api.github.com/repos/my-org/my-repo/actions/jobs/123',
          html_url:
            'https://github.com/my-org/my-repo/actions/runs/456/jobs/123',
          status: 'queued',
          conclusion: null,
          created_at: '2023-10-01T12:00:00Z',
          started_at: null,
          ...
          steps: [],
          check_run_url:
            'https://api.github.com/repos/my-org/my-repo/check-runs/123',
          labels: [],
          runner_id: null,
          runner_name: null,
          runner_group_id: null,
